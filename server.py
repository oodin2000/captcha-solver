...
# ── Tambahan handler untuk error 422 (ValidationError) ──
@app.exception_handler(RequestValidationError)
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    """Log detail error 422 dan kirim response yang jelas."""
    log.error("Validation error (422) on %s: %s", request.url, exc.errors())
    log.error("Request body: %s", await request.body())
    return JSONResponse(
        status_code=422,
